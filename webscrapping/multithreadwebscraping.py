...
def threaded_function(start, end, location, query=''):
    '''
        ...
        This function is used to thread the original get_jobs functions found in the Indeed class.
        start: 
        end: 
        location: 
        query: 
...
def multiple_threaded_function(start, end, no_of_threads, location_list, job_list=['']):
    ...
    for location in location_list:
        ...
        for job in job_list:
            ...
            print(f'{_l} / {len(location_list)}, {_l / len(location_list) * 100}% Locations Processing')
            print(f'{_j} / {len(job_list)}, {_j / len(job_list) * 100}% Positions Processing')
            print(f'{_t} / {len(location_list) * len(job_list)}, {_t / (len(location_list) / len(job_list)) * 100}% Processing')
