...
class Indeed():
    ...
    def get_data(start, end, location='India', webdriver_location='D:/A/3T_Project/chromedriver.exe'):
        ...
        for i in range(start, end):
            driver.get('https://www.glassdoor.co.in/Job/india-jobs-SRCH_IL.0,5_IN115.htm' + str(i))
            ...
            for job in driver.find_elements_by_class_name('result'):
                ...
                df = df.append(
                    {'Title': title, 'Location': location, 'Company': company, 'Salary': salary, 'Sponsored': sponsored,
                     'Description': job_desc, 'Time': time}, ignore_index=True)
